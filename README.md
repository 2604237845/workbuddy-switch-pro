# workbuddy-switch-pro

> **本仓库 = [workbuddy-switch](https://github.com/changexbc/workbuddy-switch)（上游，MIT） + 一个任务中心。**
> 上游的账号管理 App / webui **原样保留**，下方全部文档依然适用；新增的部分只有 `tasks/`。
> 源码来源与参考出处见 [§ 来源与致谢](#来源与致谢)。

## 它比上游多了什么

上游解决「账号怎么管」；本仓库补上「每日任务怎么自动做完」。两边**互补、不重复**：

| 能力 | 由谁负责 |
|---|---|
| 账号切换 · 会话复制 · 积分/Token 监控 · 自动轮换 · **每日签到** · **猫猫旅行** | 上游 workbuddy-switch |
| **成长中心 19 项**：盲盒 · 抽奖 · 连签兑换 · 补签卡 · 礼包补偿 · 徽章 · 开学季 · 小程序 · 资料库 · 主题 · 模板 · 专家团 · GLM-5.2 · 夜猫子 … 以及**自动领奖** | 本仓库新增的 `tasks/` |

两个关键设计：

- **共用同一份账号库** `~/.wb-switch/accounts.json`，任务中心**只读**它，绝不刷新 token
  （上游的 refresh token 是单链轮换的，两边同时刷会互相踢下线）。
- 上游已经做掉的两项（`t_sign` 每日签到、`t_travel` 猫猫旅行）在任务中心里被**显式排除**，
  避免重复请求。

👉 **任务中心怎么装、怎么用：[tasks/README.md](./tasks/README.md)**
👉 **想自己改任务 / 长期维护：[docs/MAINTENANCE.md](./docs/MAINTENANCE.md)**

---

## 上游 workbuddy-switch 的能力
...
本仓库不是从零写的，它站在两个开源项目之上。**来源必须写清楚**，这既是礼貌，也是 MIT 许可的要求。

### 1. 基座（账号管理部分）—— 原样保留

- **[changexbc/workbuddy-switch](https://github.com/changexbc/workbuddy-switch)** —— `crates/`、`src/`、`src-tauri/`、`npm/` 下的全部代码来自该项目。
- 许可 **MIT**，版权归 `Copyright (c) 2026 wb-switch`；原始 [LICENSE](./LICENSE) 文件**保留未改**。
- 本仓库对它的改动仅限于：改项目名与 README、新增 `tasks/` 目录。其功能代码未做修改。

### 2. 任务脚本上游（`tasks/engine/vendor/`）—— 原样保留、逐字未改

- **[L0NE-6/WorkBuddy-Daily](https://github.com/L0NE-6/WorkBuddy-Daily)** ——
  `tasks/engine/vendor/workbuddy_daily.py` 与 `workbuddy_login.py` 直接取自该项目。
- 🔴 这两个文件在本仓库里**一个字节都没动**，且**将来也不应改动**：
  任务中心是在**运行时**给它打补丁（monkey-patch），而不是改它。
  这样上游更新时整文件替换即可，永远不会产生合并冲突。
  详见 [docs/MAINTENANCE.md](./docs/MAINTENANCE.md)。
- 上游同步由 `tasks/engine/update.py` 完成，同步目标写在 `tasks/engine/config/tasks.json`
  的 `auto_update.repo`，并带体积/函数存在性/域名白名单等护栏。

### 3. 本仓库新增的部分

- `tasks/` —— 任务引擎的补丁层（`engine.py`）、常驻服务与网页面板（`tasks/service/`）、
  诊断探针（`probe_*.py`）与三套自检。
  这部分是对上面两个项目**行为差异的逆向与补齐**，其中记录了大量实测结论，
  已在源码注释里标明「实测日期 + 现象 + 证据」，方便后续维护者判断是否仍然成立。
